...
namespace BankSystem.Controller
{
    internal class TransactionController : IController
    {
        ...
        // Phương thức lấy ID giao dịch mới nhất theo loại giao dịch
        public int? GetLatestTransactionId(TransactionType transactionType)
        {
            string typeString = transactionType.ToString(); // Chuyển enum thành string

            return db.BankTransactions
                     .Where(t => t.Type == typeString)
                     .OrderByDescending(t => t.DateOfTrans)
                     .Select(t => (int?)t.Id)
                     .FirstOrDefault();
        }

        // Phương thức lấy ID giao dịch mới nhất theo ID
        public int? GetLatestTransactionId(int transactionId)
        {
            return db.BankTransactions
                     .Where(t => t.Id == transactionId) // Lọc theo ID giao dịch
